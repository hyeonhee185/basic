...
import bookstore.book.BookController;
import bookstore.common.HomeMenu;
import bookstore.common.ScannerUtil;
import bookstore.join.CustomerVO;
import bookstore.join.JoinController;
import bookstore.order.OrderController;
import bookstore.sign.SignController;
...
public class BookStoreView {
	...
	public int order(OrderController controller) {
		controller.findAll().forEach(orderVO -> {
			System.out.printf("%d\t%s\t%d\t%s\n", orderVO.getOrderId(), orderVO.getBookName(), orderVO.getSalePrice(),
					orderVO.getOrderDate());
		});
	...
	public int getBookList(BookController controller) {
		controller.selectBookList("").forEach(bookVO -> {
			System.out.printf("%d\t%s\t%s\t%d\n", bookVO.getBookId(), bookVO.getBookName(), bookVO.getPublisher(),
					bookVO.getPrice());
		});
